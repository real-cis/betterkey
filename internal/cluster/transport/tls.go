...
func NewNetTransport(config *NetTransportConfig) (*NetTransport, error) {
	...
	for _, addr := range config.BindAddrs {
		...
		tcpLn, err := tls.Listen("tcp", tcpAddr.String(), t.config.TLSConfig)

		if err != nil {
			return nil, fmt.Errorf("Failed to start TCP listener on %q port %d: %v", addr, port, err)
		}
		t.tcpListeners = append(t.tcpListeners, &Listener{listener: &tcpLn, addr: tcpAddr})
...
func (t *NetTransport) GetAutoBindPort() int {
	...
	return t.tcpListeners[0].addr.Port
}
...
func (t *NetTransport) FinalAdvertiseAddr(ip string, port int) (net.IP, int, error) {
	...
	if ip != "" {
		...
		if t.config.BindAddrs[0] == "0.0.0.0" {
			...
			advertiseAddr = t.tcpListeners[0].addr.IP
		}
...
func (t *NetTransport) IngestStream(conn net.Conn) error {
	fmt.Printf("Default IngestStream called!\n")
	t.streamCh <- conn
	return nil
}
...
func (t *NetTransport) tcpListen(tcpLn net.Listener) {
	...
	for {
		...
		if err != nil {
			...
			slog.Error("[ERR] memberlist: Error accepting TCP connection", "error", err)
